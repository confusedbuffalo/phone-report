...
function createDetailsRow(label, number) {
    return `<div class="list-item-phone-label-container">
            ...
            <div class="list-item-phone-value-container ${reportType === 'phone' ? 'break-all' : reportType === 'name' ? 'wrap-break-word' : 'break-anywhere'}">
                ${number}
            </div>`;
}
...
function createButtons(item, clickedClass) {
    ...
    if (reportType === 'name' && !item.name) {
        fixButton = Object.keys(item.nameTags)
            .map(key => {
